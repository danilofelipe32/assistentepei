        ...
        <div className="absolute top-2.5 right-2 flex flex-col space-y-1">
            ...
            {onSuggestNeedsClick && (
                 <button 
                    ...
                    className="w-7 h-7 flex items-center justify-center text-blue-500 hover:text-blue-700 hover:bg-blue-100 rounded-full transition-colors disabled:text-gray-400 disabled:hover:bg-transparent disabled:cursor-not-allowed" 
                    title="Sugerir Necessidades"
                >
                    {isSuggestNeedsLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div> : <i className="fa-solid fa-list-ul text-xs"></i>}
                </button>
            )}
            {onSuggestAdaptationsClick && (
                 <button 
                    ...
                    className="w-7 h-7 flex items-center justify-center text-orange-500 hover:text-orange-700 hover:bg-orange-100 rounded-full transition-colors disabled:text-gray-400 disabled:hover:bg-transparent disabled:cursor-not-allowed" 
                    title="Sugerir Adaptações"
                >
                    {isSuggestAdaptationsLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-orange-600"></div> : <i className="fa-solid fa-tools text-xs"></i>}
                </button>
            )}
            {isGoal && onSmartClick && (
            ...
            {(isGoal || onSuggestClick) && onSuggestClick && (
